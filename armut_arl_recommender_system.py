...
main_df = pd.read_csv('armut_data.csv')
df = main_df.copy()
...
def data_prep(df):
    ...
    # Convert CreateDate correctly (handles both day-first and mixed formats)
    df['CreateDate'] = pd.to_datetime(df['CreateDate'], dayfirst=True, errors='coerce')
...
def arl_recommender(rules_df, service, rec_count=1):
    ...
    readable_recommendations = [service_names.get(rec, f"Service {rec}") for rec in recommendation_list]
    readable_input = service_names.get(service, f"Service {service}")

    print(f"\n📦 You selected service: {readable_input}")
    print("💡 Based on past customer patterns, we recommend:")
    for rec in readable_recommendations:
        print(f"   ➤ {rec}")
